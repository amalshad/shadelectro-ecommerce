...
const loadDashboard = async (req, res) => {
    try {
        ...
        const thisMonthRevenue = thisMonthOrders.reduce((sum, order) => sum + order.finalAmount, 0);
...
const generateLedger = async (req, res) => {
    try {
        ...
        const summary = {
            ...
            totalRevenue: ledgerData.reduce((sum, order) => sum + order.finalAmount, 0),
        ...
        const formattedData = ledgerData.map(order => ({
            ...
            items: order.orderedItems.map(item =>
                `${item.product?.productName || 'Unknown'} (Qty: ${item.quantity})`
            ).join('; ')
        }));
